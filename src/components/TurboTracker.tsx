...
import { Flame, PenTool, Zap, Lock, Unlock, Crown } from "lucide-react";
import { Progress } from "@/components/ui/progress";
import { Badge } from "@/components/ui/badge";
import { cn } from "@/lib/utils";
import { canAccessTurbo, getPlanDisplayName } from "@/lib/plans";
...
export function TurboTracker({
  ...
  const hasPlanAccess = canAccessTurbo(plan);
    ...
    return (
      <div className="space-y-2 px-2 py-2">
        ...
        <div className="flex items-center justify-between text-xs mt-1">
          ...
          <span className={cn("font-semibold text-xs", hasPlanAccess && turboUnlocked ? "text-amber-500" : "text-muted-foreground")}>
            {!hasPlanAccess
              ? "Pro+ only"
              : turboUnlocked
              ? `${(turboWordsRemaining / 1000).toFixed(0)}K left`
              : "Locked"}
          </span>
        </div>
        {hasPlanAccess && turboUnlocked && <Progress value={turboWordsProgress} className="h-1.5" variant="accent" />}
      </div>
    );
  }
      ...
      <div className="rounded-lg border p-4 space-y-3">
        ...
        {!hasPlanAccess ? (
          <p className="text-xs text-muted-foreground">
            ...
            Upgrade to Pro or Elite to unlock Turbo Mode and Auto Draft.
          </p>
        ) : turboUnlocked ? (
          <div className="space-y-1">
            <Progress value={turboWordsProgress} className="h-2" variant="accent" />
            <p className="text-xs text-muted-foreground">
              Auto Draft mode available.{plan === "elite" ? " Elite: faster accumulation & reduced decay." : " Turbo words refill each cycle."}
            </p>
          </div>
        ) : (
          <p className="text-xs text-muted-foreground">
