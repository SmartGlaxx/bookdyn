...
const CreateBookEngine = ({ onClose, onCreate }: CreateBookEngineProps) => {
  ...
  const [formData, setFormData] = useState<Partial<CreateBookInput>>({
    ...
    controls: { ...getDefaultControls("novel"), automationLevel: "" as AutomationLevel, depthLevel: "" as DepthLevel, temporalContext: { era: "" as TemporalEra, timelineStructure: "" as TimelineStructure }, structureControls: { ...getDefaultControls("novel").structureControls, chapterCount: "" as any, subsectionCount: "" as any } },
  });
  ...
  const handleBookTypeChange = (type: BookType) => {
    ...
    updateForm("controls", { ...getDefaultControls(type), automationLevel: "" as AutomationLevel, depthLevel: "" as DepthLevel, temporalContext: { era: "" as TemporalEra, timelineStructure: "" as TimelineStructure }, structureControls: { ...getDefaultControls(type).structureControls, chapterCount: "" as any, subsectionCount: "" as any } });
                  ...
                  <Section emoji="📐" title="Structure">
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <FieldGroup label="Sections/Chapter" tooltip="How many sections each chapter contains">
                        <Select
                          value={formData.controls?.structureControls?.subsectionCount || ""}
                          onValueChange={(v) => updateStructureControls("subsectionCount", v)}
                        >
                          <SelectTrigger className="text-xs"><SelectValue placeholder="Select" /></SelectTrigger>
                          <SelectContent className="bg-popover z-50">
                            <SelectItem value="flexible">Flexible</SelectItem>
                        ...
                        {formData.controls?.structureControls?.subsectionCount === "fixed" && (
                          <Input
                            type="number"
                            min={2}
                            max={10}
                            ...
                            value={formData.controls?.structureControls?.targetSubsections || 4}
                            onChange={(e) =>
                              updateStructureControls("targetSubsections", Math.max(2, Math.min(10, parseInt(e.target.value) || 4)))
                            }
                          />
                        )}
                      </FieldGroup>
